...
def main(
    ...
    for d in out_data_arr:
        ...
        responses.append(d[extract_key].squeeze(0))  # Pre cos/sin 1x1 conv
    ...
    if analysis == "feature_select":
        ...
        if basis_function == "step":
            ...
            gaussian = stats.norm(loc=90, scale=60).pdf(np.arange(180))  # noqa
            for i in range(180):
        ...
        if model is None:
            ...
            # Get parameters for mapping responses to each row of indicator_matrix
            X = responses.T
            clfs = []
            import ipdb;ipdb.set_trace()
            for y in indicator_matrix:
                clf = model.fit(X, y)  #
                clfs.append(clf.coef_)
            clfs = np.asarray(clfs)
            import ipdb;ipdb.set_trace()
            clf = clfs
